...
<div class="flex flex-col md:flex-row min-h-screen">
    ...
    <div class="md:w-1/3 min-h-screen bg-purple-900 text-white flex flex-col justify-center items-center gap-6 p-6">
        ...
        <div class="flex flex-col justify-around items-center w-full max-w-md space-y-8">
            <!-- Number of Participants -->
            <div class="flex flex-col items-center w-full ">
                <label for="participants" class="text-lg font-medium ">Number of Participants:</label>
                <input type="number" id="participants" bind:value={roomSettings.maxPlayers} class="w-2/3 text-black text-center px-3 py-2 bg-gray-50 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring focus:border-indigo-500 sm:text-sm" placeholder="Enter number of participants">
            </div>
            ...
            <div class="flex justify-center items-center gap-4 mt-2 w-full space-x-4">
                {#each bgMusic.slice(0, bgMusic.length - 1) as music, index}
                    ...
                    <div 
                        ...
                        class="cursor-pointer font-bold text-lg p-2 bg-purple-700 rounded {index === bgMusicIdx ? 'ring-2 ring-purple-600' : ''}"
                    >
                        {index + 1}
                    </div>
                {/each}
                ...
                <div 
                    ...
                    class="cursor-pointer font-bold text-lg p-2 bg-purple-700 rounded {bgMusicIdx === bgMusic.length - 1 ? 'ring-2 ring-purple-600' : ''}"
                >
            ...
            <div class="flex flex-col items-center w-full">
                <label for="scoreDisplaySlider" class="text-lg font-medium">Score Display Frequency:</label>
                <input 
                    type="range" 
                ...
                >
                <span class="text-sm">
                    {roomSettings.reportScores === -1 ? 'No score display during quiz' : roomSettings.reportScores === 1 ? `After every ${roomSettings.reportScores} question` : `After every ${roomSettings.reportScores} questions`}
                </span>
            </div>
    ...
    <div class="md:w-2/3">
        <DmyGame {selectedBgColor} {selectedBgMusic} {showToggleQuestionDisplay} {displayQuestionOnPlayer}/>
    </div>
