<script>
  ...
  import {
    HISTORY_DISPLAY_PAGINATION_HOST,
    HISTORY_DISPLAY_PAGINATION_PLAYER,
  } from "$lib/config";
  ...
  function scrollToContent() {
    ...
    if (section) {
      section.scrollIntoView({ behavior: "sudden" });
    }
  }
  ...
  // Optionally, you can hide the prompt when user starts scrolling
  $: if (showPlayerHistory || showHostHistory) {
    window.onscroll = () => {
...
<main class="font-garamond">
  {#if showScrollPrompt}
    <div class="fixed bottom-10 w-full text-center">
      <button
        class="text-white bg-purple-800 py-2 px-4 rounded-full cursor-pointer"
  ...
  {:else}
    <body class="bg-purple-500">
      <div class="bg-purple-900 text-white py-4">
        <div class="navbar mx-auto px-4 flex justify-between items-center">
          <div>
            <a
              href="/hostOrPlayer"
              class="bg-gray-500 hover:bg-gray-600 text-white font-bold py-3 px-6 rounded-full transition duration-300 transform hover:scale-105 shadow-lg mt-4"
              >Back</a
            >
          </div>
      ...
      <div
        class="mr-8 mt-[-200px] flex justify-center items-center h-screen"
        id="contentSection"
      >
        <button
          on:click={() => togglePlayerHistory(playerEmail)}
          class="w-4/9 bg-gradient-to-r from-purple-700 to-purple-900 hover:from-purple-600 hover:to-purple-800 text-white font-bold py-3 px-6 rounded-full transition duration-300 transform hover:scale-105 shadow-lg{fadePlayedButton
            ? 'opacity-100'
            : ''}"
          id="contentSection">Played</button
        >
        <div class="mx-12" />
        <button
          on:click={() => toggleHostHistory(playerEmail)}
          class="bg-gradient-to-r from-purple-700 to-purple-900 hover:from-purple-600 hover:to-purple-800 text-white font-bold py-3 px-6 rounded-full transition duration-300 transform hover:scale-105 shadow-lg{fadeHostedButton
            ? 'opacity-50'
            : ''}"
          id="contentSection">Hosted</button
        >
      </div>
      ...
      {:else}
        <div
          class="mr-6 mt-[300px] flex flex-col items-center min-h-screen bg-black bg-opacity-80 p-8 rounded-lg shadow-2xl text-center w-full"
        >
          {#if showPlayerHistory}
            {#each detailedPlayerHistory as quiz}
              <div class="w-full md:w-1/2 mb-8">
                <div
                  class="bg-black bg-opacity-80 p-8 bg-purple-500 p-4 rounded shadow-md"
                >
          ...
          {#if !loadedDefault_Player && showPlayerHistory}
            <!--Pagination-->
            <div class="flex justify-center items-center mt-8 font-garamond">
              <button
                class="bg-gradient-to-r from-purple-700 to-purple-900 hover:from-purple-600 hover:to-purple-800 text-white font-bold py-3 px-6 rounded-full transition duration-300 transform hover:scale-105 shadow-lg"
                on:click={PrevPage_Player}>Previous</button
              >
              <span class="mx-4 text-lg text-white"
                >Page {currentpage_Player} of {totalpage_Player}</span
              >
              <button
                class="bg-gradient-to-r from-purple-700 to-purple-900 hover:from-purple-600 hover:to-purple-800 text-white font-bold py-3 px-6 rounded-full transition duration-300 transform hover:scale-105 shadow-lg"
                on:click={NextPage_Player}>Next</button
              >
            </div>
          {/if}
        </div>
      ...
      {#if pageLoading[2]}
        <div
          class="fixed inset-0 bg-purple-500 flex justify-center items-center"
        >
          <div class="loader" />
        </div>
      {:else}
        <div class="mr-6 mt-[-900px] flex flex-col items-center min-h-screen">
          {#if showHostHistory}
            <div class="flex flex-col space-y-4">
              {#each hostHistory as quiz}
                <div class="relative">
                  ...
                  <div
                    class="bg-purple-800 text-white p-4 rounded shadow-md relative"
                  >
                    <h2 class="text-2xl font-bold mb-6">
                      {quiz.quiz} Quiz Scores
                    </h2>
                  </div>
                  <div
                    class="bg-purple-500 p-4 rounded-b shadow-md relative z-10"
                  >
          ...
          {#if !loadedDefault_Host && showHostHistory}
            ...
            <div class="flex justify-center items-center mt-8">
              <button
                class="bg-gradient-to-r from-purple-700 to-purple-900 hover:from-purple-600 hover:to-purple-800 text-white font-bold py-3 px-6 rounded-full transition duration-300 transform hover:scale-105 shadow-lg"
                on:click={PrevPage_Host}>Previous</button
              >
              <!--display curr page / total -->
              <span class="mx-4 text-white"
                >{currentpage_Host}/{totalpage_Host}</span
              >
              <button
                class="bg-gradient-to-r from-purple-700 to-purple-900 hover:from-purple-600 hover:to-purple-800 text-white font-bold py-3 px-6 rounded-full transition duration-300 transform hover:scale-105 shadow-lg"
                on:click={NextPage_Host}>Next</button
              >
            </div>
          {/if}
        </div>
